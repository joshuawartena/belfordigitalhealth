...
class GBPChecker:
    ...
    def _check_category_match(self, cat: CategoryResult, place: dict, audit: AuditResult):
        ...
        # The Places API sometimes returns a generic type (e.g. 'service') even when the GBP
        # listing has a specific category. Fall back to matching the display name in that case.
        _GENERIC_API_TYPES = {'service', 'establishment', 'point_of_interest'}

        expected_label = GBP_CATEGORY_LABELS.get(expected, expected.replace('_', ' ').title())
        norm_expected_type = expected.lower().strip()
        norm_type = primary_type.lower().strip()
        norm_display = display_name.lower().strip()
        norm_expected_label = expected_label.lower().strip()

        type_matches = norm_type == norm_expected_type
        display_matches = bool(norm_display) and (
            norm_display == norm_expected_label
            or norm_expected_label in norm_display
            or norm_display in norm_expected_label
        )
        api_is_generic = norm_type in _GENERIC_API_TYPES

        if type_matches or display_matches:
            cat.add(
                name='Category Match', status='pass', priority=pri,
                points_earned=pts, points_possible=pts,
                value=expected_label,
                detail=(
                    f'GBP primary category "{label}" matches the expected '
                    f'"{expected_label}" for {detected_brand or "this brand"}.'
                ),
            )
        elif api_is_generic:
            cat.add(
                ...
                value=label,
                detail=(
                    f'Places API returned generic category "{label}" — could not confirm '
                    f'expected "{expected_label}". Verify the GBP dashboard shows '
                    f'"{expected_label}" as the primary category.'
                ),
                recommendation=(
                    f"CATEGORY VERIFICATION NEEDED: The Places API returned a generic category type "
                    f"for this listing, so the expected '{expected_label}' category could not be confirmed automatically. "
                    f"Log into the GBP dashboard and verify the primary category is set to '{expected_label}'."
                ),
            )
        elif norm_expected_type in norm_type or norm_type in norm_expected_type:
            cat.add(
                name='Category Match', status='warn', priority=pri,
                points_earned=round(pts * 0.5, 1), points_possible=pts,
                value=label,
                ...
                recommendation=(
                    f"CATEGORY REFINEMENT: Your GBP primary category is '{label}', which partially "
                    f"matches the expected '{expected_label}'. Verify the exact category is set correctly "
            ...
            cat.add(
                ...
                recommendation=(
                    ...
                    f"category for this franchise brand is '{expected_label}'. Log in to your GBP dashboard and "
                    f"change the primary category to '{expected_label}'."
                ),
            )
